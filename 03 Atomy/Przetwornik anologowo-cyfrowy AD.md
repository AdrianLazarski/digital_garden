---
created: 2026-03-27 16:03
tags:
  - atom
  - definicja
  - it/technika_cyfrowa
dojrzalosc: Sadzonka 🌱
status_epistemiczny: Spekulatywny ❓
modified date: 2026-03-27 16:03
---
ang. *analog-to-digital converter*

> [!note] Definicja
> Układ służący do zamiany [[Sygnał analogowy|sygnału analogowego]] na [[sygnał cyfrowy]].

Jego działanie polega na próbkowaniu [[sygnał|sygnału]] (mierzeniu wartości w czasie) oraz kwantyzacji (przypisaniu wartości do najbliższego poziomu cyfrowego).

Jeśli się chce odsłuchać zapisany tak cyfrowo sygnał audio, trzeba dokonać odwrotnej konwersji za pomocą [[Przetwornik cyfrowo-analogowy DA|przetwornika cyfrowo-analogowego DA]].


![[Konwersja sygnału AD i DA.webp|Konwersja sygnału AD i DA]]


- Wartość sygnału (napięcia) jest próbkowana (mierzona) w n chwilach czasowych z częstotliwością co najmniej dwa razy większą niż częstotliwość sygnału. Spróbkowane wartości są poddawane kwantyzacji, to znaczy, że ich wartość jest zamieniana na najbliższą wartość, którą można zapisać w postaci m-bitowej liczby binarnej.
- Częstotliwość próbkowania i liczba bitów zależą od jakości zastosowanego przetwornika A/D.

![[Próbkowanie.webp|Próbkowanie]]

